...
def __main__():
    ...
    print(np.transpose(total_corrs))

    corrs_mean_0 = np.delete(np.array(corrs_mean)[0], 0)
    corrs_mean_1 = np.delete(np.array(corrs_mean)[1], 1)
    corrs_mean_2 = np.delete(np.array(corrs_mean)[2], 2)
    corrs_mean_3 = np.delete(np.array(corrs_mean)[3], 3)
    print(np.array(corrs_mean))
    corrs_mean = np.array([corrs_mean_0, corrs_mean_1, corrs_mean_2, corrs_mean_3])

    z_mean_calc = calcZ(corrs_mean)
    z_sd_calc = 1 / np.sqrt(sims - 3)

    z_stat = (-1) * stats.norm.ppf(0.025)

    z_min = z_mean_calc - (z_stat * z_sd_calc / np.sqrt(sims))
    z_max = z_mean_calc + (z_stat * z_sd_calc / np.sqrt(sims))

    r_min = calcR(np.array(z_min)).flatten()
    r_max = calcR(np.array(z_max)).flatten()

    cov_min_petr_wege = r_min[0 ] * np.sqrt(petr_var) * np.sqrt(wege_var)
    cov_min_petr_abev = r_min[1 ] * np.sqrt(petr_var) * np.sqrt(abev_var)
    cov_min_petr_vale = r_min[2 ] * np.sqrt(petr_var) * np.sqrt(vale_var)
    cov_min_wege_petr = r_min[3 ] * np.sqrt(wege_var) * np.sqrt(petr_var)
    cov_min_wege_abev = r_min[4 ] * np.sqrt(wege_var) * np.sqrt(abev_var)
    cov_min_wege_vale = r_min[5 ] * np.sqrt(wege_var) * np.sqrt(vale_var)
    cov_min_abev_petr = r_min[6 ] * np.sqrt(abev_var) * np.sqrt(petr_var)
    cov_min_abev_wege = r_min[7 ] * np.sqrt(abev_var) * np.sqrt(wege_var)
    cov_min_abev_vale = r_min[8 ] * np.sqrt(abev_var) * np.sqrt(vale_var)
    cov_min_vale_petr = r_min[9 ] * np.sqrt(vale_var) * np.sqrt(petr_var)
    cov_min_vale_wege = r_min[10] * np.sqrt(vale_var) * np.sqrt(wege_var)
    ...
    cov_max_petr_vale = r_max[2 ] * np.sqrt(petr_var) * np.sqrt(vale_var)
    cov_max_wege_petr = r_max[3 ] * np.sqrt(wege_var) * np.sqrt(petr_var)
    cov_max_wege_abev = r_max[4 ] * np.sqrt(wege_var) * np.sqrt(abev_var)
    cov_max_wege_vale = r_max[5 ] * np.sqrt(wege_var) * np.sqrt(vale_var)
    cov_max_abev_petr = r_max[6 ] * np.sqrt(abev_var) * np.sqrt(petr_var)
    cov_max_abev_wege = r_max[7 ] * np.sqrt(abev_var) * np.sqrt(wege_var)
    cov_max_abev_vale = r_max[8 ] * np.sqrt(abev_var) * np.sqrt(vale_var)
    cov_max_vale_petr = r_max[9 ] * np.sqrt(vale_var) * np.sqrt(petr_var)
    cov_max_vale_wege = r_max[10] * np.sqrt(vale_var) * np.sqrt(wege_var)
